...
def get_ws():
    ...
    if _ws is None:
        ...
        log.info("Google Sheets connected OK — tab: %s", SHEET_TAB)
    return _ws

def append_row(row: list) -> int:
    ws     = get_ws()
...
_SYSTEM = (
    ...
    "Rules: direction defaults to OUT unless user explicitly states income/received/IN. "
    "Use today's date if none found. Match all dropdown fields strictly to the allowed values only."
)
...
class GeminiParser:
    ...
    def parse_text(self, text):
        return self._build(self._clean(
            self.model.generate_content(self._prompt(f"User input:\n{text}")).text))

    def parse_image(self, image_bytes):
        img = PIL.Image.open(io.BytesIO(image_bytes))
        return self._build(self._clean(
            self.model.generate_content([self._prompt("Extract from this receipt image."), img]).text))

    def parse_pdf(self, raw):
        pdf  = fitz.open(stream=raw, filetype="pdf")
        text = "\n".join(p.get_text() for p in pdf)[:4000]
        return self._build(self._clean(
            self.model.generate_content(self._prompt(f"PDF text:\n{text}")).text))
...
async def handle_text(u, c):
    ...
    if pending_step.get(uid) == "edit_description" and uid in pending:
        pending[uid].description = u.message.text.strip()
        pending_step[uid] = None
        await advance(uid, c, u.effective_chat.id); return
    await u.message.reply_text("Parsing...")
    try:
        t = parser.parse_text(u.message.text)
        pending[uid] = t
...
async def handle_photo(u, c):
    ...
    try:
        ...
        t = parser.parse_image(bytes(await f.download_as_bytearray()))
        pending[uid] = t
...
async def handle_document(u, c):
    ...
    try:
        ...
        if not t:
            await u.message.reply_text("Unsupported type. Send PDF or image."); return
        pending[uid] = t
        await u.message.reply_text(f"Found: {t.direction} {t.raw_amount} {t.currency} — {t.description}")
        await advance(uid, c, u.effective_chat.id)
    except Exception as e:
...
async def handle_callback(u, c):
    ...
    if   prefix == "currency":     t.currency     = value
        ...
        m = {"description": ("edit_description", "Type new description:"),
             "direction":   (None, None), "currency": (None, None),
             "bank":        (None, None), "category": (None, None),
             "sub_category":(None, None), "name":     (None, None),
             "project":     (None, None), "company":  (None, None)}
        if value == "description":
            pending_step[uid] = "edit_description"
            await q.edit_message_text("Type the new description:")
